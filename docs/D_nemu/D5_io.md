...
> **所属阶段**: D阶段（基础阶段）
> **原始来源**: [https://ysyx.oscc.cc/docs/2407/d/5.html](https://ysyx.oscc.cc/docs/2407/d/5.html)
> **爬取时间**: 2026-03-27
...
我们知道RISC-V处理器通过MMIO访问外设, 例如在NEMU中串口会映射到0xa00003f8. 类似地, 我们也可以在NPC的仿真环境中实现简单的串口和时钟. 在上一节中提到, 我们通过DPI-C方式让NPC调用读写函数pmem_read()和pmem_write()来访问内存. 和NEMU一样, 我们可以在这两个函数中添加若干判断来实现MMIO的功能, 伪代码如下所示:
