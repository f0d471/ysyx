...
## 四·补　合并的边界：CTE 真的与平台无关吗？

合并的理由是"陷入与上下文切换是纯 ISA 行为"。这个论断被质疑过——**B 阶段接入
ysyxSoC 后，bootloader 会让链接脚本、`trap.S` 在 NEMU 与 NPC 上表现不同**。
质疑是对的方向，但落点不在 `trap.S`。对着讲义逐条核过：

### 讲义原文

**B2「为ysyxSoC添加AM运行时环境」**：

> 噢, 这不就是**实现一个新的AM**吗?

随后列出的四项全部属于 TRM：堆区放 SRAM、入口链接到 MROM 且首指令与复位 PC 对齐、
`halt` 用 `ebreak`、`putch` 走 UART16550。

**B2「通过bootloader将数据段加载到内存」**：数据段的 LMA 在 MROM、VMA 在 SRAM，
靠链接脚本的 `> mrom AT> mrom` 区分，启动时 `memcpy` 搬运。

→ **落点是 `start.S` / `trm.c` / `linker.ld` / `.mk`，而这四处一处都没有合并**：
`riscv/{nemu,npc}/start.S` 仍是两份，`trm.c` 仍是两份，链接脚本与 mk 本就分平台，
`vme.c` 更是 npc 用 `platform/dummy/` 而 nemu 用自己的。

**C5「异常处理」**：

> 目前我们只需要实现 `ecall` 这一种异常, 只需要同时设置mcause和mepc, 然后跳转到
> mtvec中存放的异常入口即可.
>
> 虽然RISC-V手册中对mstatus的介绍功能繁多, 但目前我们都不需要使用, **只需要正确
> 初始化通过DiffTest即可**.

→ 纯 ISA 描述；后半句还反证了 `trap.S` 里那段 MPRV 是**参照实现的产物而非平台功能**。

### 但"完完全全是 ISA 级别"这个说法过强

现在只有 MPRV 一处差异，将来有四个可能分化的点，提前记下：

1. **`__am_asm_trap` 的落址**。代码段链在 MROM（XIP，读取慢）时，每次异常都要走
   flash/mrom 取指。要优化就得给 trap 入口加 section 属性配合链接脚本——**这是
   链接脚本与 `trap.S` 之间真实存在的耦合点**，也最接近上述质疑所指。
2. **`mtvec` 的 MODE 位**。低 2 位选 Direct/Vectored。两边现在都是 Direct。
3. **中断**（C 阶段 RT-Thread）。`mcause` 的编码是 ISA 的，但**"哪些中断源存在"
   是平台的**——有没有 CLINT 决定了 `ienabled`/`iset` 能不能真做事。
4. **Access Fault**。B2 建议 NPC 实现它，届时 `mcause` 会出现 5/7。两边都加无害。

### 结论：这些是"需要平台差异的表达机制"，不是"需要两份拷贝"

一个 `#ifdef` 好过两份会漂移的文件——**本文记录的缺陷正是两份拷贝造成的，而不是
一个 `#ifdef` 造成的**。为此顺带补上的 `ARCHFLAGS`（让 `ASFLAGS` 也能看到平台宏）
就是这个机制本身。而且 B2 说的是**新增一个 AM 平台**，届时 ysyxsoc 可以自行决定
复用共享实现还是另写一份，合并没有堵死这条路。

### 何时该拆回去

出现下列任一情况时，合并就不再划算，应当拆分：

- `trap.S` 或 `cte.c` 里的 `#ifdef __PLATFORM_*` 超过 **3 处**，或出现嵌套；
- 某个平台需要**改变控制流结构**（而不只是增删几条指令），例如中断与异常走不同入口；
- 出现第三个 RISC-V 平台且它与现有两者都不同构。

### ⚠️ 由这次讨论修正的一处：守卫要用白名单

MPRV 的条件最初写成 `#ifndef __PLATFORM_NPC__`（黑名单）。这是错的方向：将来出现
`riscv32e-ysyxsoc` 时 `PLATFORM=ysyxsoc`，条件为真，**新平台会静默继承 MPRV**，
而它跑的是自研 CPU、CSR 不保证有第 17 位。

往未实现的 CSR 位写值是**安静的硬件误动作**；difftest 不匹配则是**响亮的、可查的
失败**。默认应当倒向后者。改为白名单：

```c
#if defined(__PLATFORM_NEMU__) || defined(__PLATFORM_LOGISIM__)
```

已逐架构核对，7 个 RISC-V 架构的行为与合并前完全一致（nemu 系 4 个 + logisim 设置，
npc 系 2 个不设置）。

> 📌 **通用原则：条件编译的守卫要表达"谁需要"，而不是"谁被排除"。** 白名单让新成员
> 默认落在保守的一侧，黑名单让新成员默认继承它从未被验证过的行为。
