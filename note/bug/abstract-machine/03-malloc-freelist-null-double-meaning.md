...
## 〇、前置知识：动态内存分配器是怎么运作的

### `malloc` / `free` 是什么

**`malloc` = memory allocate，动态内存分配**；**`free`**，释放。

```c
void *malloc(size_t size);   // 要一块至少 size 字节的内存，失败返回 NULL
void  free(void *ptr);       // 把之前要到的那块还回去
```

- 返回 `void *`（"指向任意类型的指针"），拿到手要自己转成需要的类型；
- **返回的内存内容是未定义的**，不会自动清零（想清零用 `calloc`）；
- **失败时返回 `NULL`**，调用者必须检查——这是本文缺陷的关键接口约定。

### 为什么需要它：三种内存的分工

| 区域 | 中文 | 谁管 | 生命周期 |
| --- | --- | --- | --- |
| static/global | 静态区 | 编译期定死 | 整个程序 |
| stack | **栈** | 编译器自动 | 函数返回即消失 |
| heap | **堆** | **程序员手动** | 从 `malloc` 到 `free` |

当"要多大"只有运行时才知道（读进来一张图多大、用户输入多少条记录），就只能用堆。代价是**必须自己记账**——分配器（allocator）就是那个记账的东西。

### 分配器要解决的核心问题

给它一大片连续内存（本项目里是 `heap.start` 到 `heap.end`，128MB），它要能：

1. 切出任意大小的小块交给调用者；
2. 记住哪些切出去了、哪些还空着；
3. 还回来的块要能**重新利用**，否则内存会越用越少。

### 这个分配器用的三个经典手法

**① 块头（block header）——把账记在内存自己身上**

分配器不另开一张表，而是在每块内存**前面**塞一个小结构体记录元信息：

```
       ┌──────────────┬────────────────────────┐
       │  BlockHeader │   给调用者的实际内存    │
       │  size, next  │                        │
       └──────────────┴────────────────────────┘
       ↑                ↑
       实际块首         malloc 返回的指针
...
所以 `malloc(100)` 实际要占掉 100 + 8 字节。反过来，`free(ptr)` 只要把指针**往回退 8 字节**就能找到这块的账本——本项目正是这么做的：

```c
BlockHeader *blk = (BlockHeader *)((char *)ptr - MHDR_SZ);
```

**② 空闲链表（free list）——把空闲块串起来**

所有还没被分配的块用指针串成一条链，链头存在全局变量 `free_list` 里。分配就是"从链上摘一块"，释放就是"往链上插一块"。

> ⚠️ **本文的缺陷正出在这个 `free_list` 变量身上**：它既被当成链表头，又被拿来判断"初始化了没有"。

**③ 首次适应（first-fit）——怎么挑块**

从链头开始遍历，**遇到的第一个够大的块就用它**，不再往后找。

| 策略 | 中文 | 做法 | 特点 |
| --- | --- | --- | --- |
| first-fit | 首次适应 | 第一个够大的 | 快，但前部容易碎 |
| best-fit | 最佳适应 | 最接近需求的 | 浪费少，但要遍历全表 |
| worst-fit | 最坏适应 | 最大的那块 | 留下的残块较大 |

### 两个配套动作：分裂与合并

**分裂（split）**：找到的块比需要的大很多时，切成两半，前半给出去，后半留在链上。本项目的规则是：

```c
if (remain >= MHDR_SZ + 8) { 分裂 }     // 剩余不足 16 字节就整块给出去，不切
```

> 这条规则很重要——它保证了**链上任何空闲块都 ≥ 16 字节**，第五节测试用的关键不变量就建立在它上面。

**合并（coalesce）**：释放时，如果前后邻居也是空闲的，就并成一块大的。否则内存会碎成一堆用不了的小块（**碎片化**，fragmentation）。

### 标准怎么规定（C99 §7.20.3.3）

- 返回的指针必须**适当对齐**，能存放任何类型的对象；
- `size` 为 0 时，返回 NULL 或一个唯一的、不可解引用的指针（实现自选）；
- **分配失败返回空指针**；
- 相关条款：`malloc` §7.20.3.3、`free` §7.20.3.2、`calloc` §7.20.3.1、`realloc` §7.20.3.4。
